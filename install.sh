...
' | sudo -E tee /etc/nginx/sites-available/$server_name.conf >/dev/null 2>&1
    ...
    # create database
    Q1="CREATE DATABASE IF NOT EXISTS workW12;"
    Q2="GRANT ALL ON *.* TO 'workW12'@'localhost' IDENTIFIED BY '$password';"
    Q3="FLUSH PRIVILEGES;"
    SQL="${Q1}${Q2}${Q3}"
    sudo mysql -u root -p="" -e "$SQL"
    
    # create stratum user
    Q1="GRANT ALL ON *.* TO 'workW12'@'localhost' IDENTIFIED BY '$password2';"
    Q2="FLUSH PRIVILEGES;"
    SQL="${Q1}${Q2}"
    sudo mysql -u root -p="" -e "$SQL"  
...
define('"'"'YAAMP_DBNAME'"'"', '"'"'workW12'"'"');
define('"'"'YAAMP_DBUSER'"'"', '"'"'workW12'"'"');
...
sudo sed -i 's/database = yaamp/database = workW12/g' *.conf
sudo sed -i 's/username = root/username = workW12/g' *.conf
